...
- **排序**：照片按文件名（不含扩展名）做字典序排序，排序越靠后的文件展示越靠前，即文件名字典序较大的照片排在更上面。例如 `20260907wm2.jpg` 会排在 `20260818wm0.png` 前面。
- **排列**：照片以网格形式按行优先排列，从左到右填满一行后再换到下一行。
- **列宽**：每列最小宽度为 240px；屏幕宽度小于 768px 时降为 180px，小于 480px 时降为 140px，列数由容器宽度自动决定。
- 请保证同一相册内照片文件名不重复，否则排序结果不确定。
- `cover.jpg` / `cover.webp` 仅作为封面，不参与照片列表。
